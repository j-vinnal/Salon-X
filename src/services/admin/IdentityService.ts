...
export class IdentityService extends BaseService {
  ...
  async editAccount(id: string, data: IEditAccountRequest, jwtData: IJWTResponse): Promise<IResultObject<IJWTResponse>> {
    try {

      const response = await this.axios.put(`editAccount/${id}`, data, {
        headers: {
  ...
  async editPassword(id: string, data: IEditPasswordRequest, jwtData: IJWTResponse): Promise<IResultObject<IJWTResponse>> {
    try {
      const response = await this.axios.put(`editPassword/${id}`, data, {
        headers: {
          Authorization: 'Bearer ' + jwtData.jwt,
        },
      });
